...
def main(mode, mode_count, button_overlay, delete_count, delete_button_overlay, s2_lst_remove):
    ...
    # cap = cv2.VideoCapture("dataset/test_video/한글 지문자 모음.mp4")
        ...
        if result.multi_hand_landmarks is not None:
            ...
            if 100 < x1 < 540 and 100 < y1 < 400:
                ...
                # Number mode
                    ...
                    number_seq.append(action)

                    if len(number_seq) < 10:
                        continue

                    this_action = ' '
                    number_dict = {}
                    for number in number_seq:
                        number_dict[number] = number_seq.count(number)
                    number_dict = Counter(number_dict).most_common()
                
                    if number_dict[0][1] > 10:
                        this_action = number_dict[0][0]
                        number_dict = {}
                        if last_action != this_action:
                            last_action = this_action
        else:
